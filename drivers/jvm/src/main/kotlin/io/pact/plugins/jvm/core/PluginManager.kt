...
object DefaultPluginManager : KLogging(), PluginManager {
    ...
    private fun lookupPlugin(name: String, version: String?): PactPlugin? {
        return if (version == null) {
            PLUGIN_REGISTER.filter {
                it.value.manifest.name == name
            }.entries.maxByOrNull { it.value.manifest.version }?.value
        } else {
        ...
        return when (matcher) {
            is CatalogueContentMatcher -> {
                ...
                val request = Plugin.CompareContentsRequest.newBuilder()
                    ...
                    .build()
                val plugin = lookupPlugin(matcher.pluginName, null) ?: throw PactPluginNotFoundException(
                    matcher.pluginName,
                    null
                )
                ...
                throw RuntimeException("Mis-configured content type matcher $matcher")
            }
        }
    }
    ...
    override fun configureContentMatcherInteraction(
        ...
        val request = Plugin.ConfigureInteractionRequest.newBuilder()
            ...
            .build()
        val plugin =
            lookupPlugin(matcher.pluginName, null) ?: throw PactPluginNotFoundException(matcher.pluginName, null)
        ...
        return if (response.error.isNotEmpty()) {
            ...
            for (interaction in response.interactionList) {
                ...
                val metadata = if (interaction.hasMessageMetadata()) {
                    interaction.messageMetadata.fieldsMap.entries.associate { (key, value) ->
                        key to valueToJson(value)
                    }
                } else {
                    emptyMap()
                }
    ...
    override fun generateContent(
        ...
        val plugin =
            lookupPlugin(contentGenerator.catalogueEntry.pluginName, null) ?: throw PactPluginNotFoundException(
                contentGenerator.catalogueEntry.pluginName,
                null
            )
        ...
        val request = Plugin.GenerateContentRequest.newBuilder()
            ...
            .setTestMode(
                if (testMode == GeneratorTestMode.Consumer)
                    Plugin.GenerateContentRequest.TestMode.Consumer
                else Plugin.GenerateContentRequest.TestMode.Provider
            )
            .setContentFor(
                if (forRequest) Plugin.GenerateContentRequest.ContentFor.Request
    ...
    override fun prepareValidationForInteraction(
        ...
        return Result.Ok(InteractionVerificationData(
            OptionalBody.body(
                response.interactionData.body.content.value.toByteArray(),
                ContentType(response.interactionData.body.contentType)
            ),
    ...
    override fun verifyInteraction(
        ...
        val request = Plugin.VerifyInteractionRequest.newBuilder()
            .setInteractionData(
                Plugin.InteractionData.newBuilder()
                    .setBody(
                        Plugin.Body.newBuilder()
                            .setContent(
                                BytesValue.newBuilder()
                                    .setValue(ByteString.copyFrom(verificationData.requestData.value)).build()
                            )
            ...
            Result.Ok(InteractionVerificationResult(response.result.success, response.result.mismatchesList.map {
                if (it.hasError()) {
                    ...
                    InteractionVerificationDetails.Mismatch(
                        it.mismatch.expected.value.toByteArray(),
    ...
    private fun startPluginProcess(
        ...
        return try {
            ...
            val timeout = System.getProperty("pact.plugin.loadTimeoutInMs")?.toLongOrNull() ?: DEFAULT_LOAD_TIMEOUT_MS
            ...
            if (startupInfo is JsonObject) {
                Result.Ok(
                    DefaultPactPlugin(
                        cp,
                        manifest,
                        toInteger(startupInfo["port"]),
                        toString(startupInfo["serverKey"])!!
                    )
                )
            } else {
                cp.destroy()
                Result.Err(
                    """Plugin process did not output the correct startup
                    | message in $timeout ms - got $startupInfo""".trimMargin()
                )
            }
    ...
    fun loadPluginManifest(name: String, version: String?): Result<PactPluginManifest, String> {
        ...
        return if (manifest != null) {
            ...
            if (manifestList.isNotEmpty()) {
                ...
                Result.Err(
                    """No plugin with name '$name' and version '${version ?: "any"}'
                      | was found in the Pact plugin directory '$pluginDir'""".trimMargin()
                )
            }
        }
    }
