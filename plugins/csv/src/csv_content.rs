...
use pact_plugin_driver::utils::{proto_struct_to_map, to_proto_struct};
      ...
      for (key, value) in &config.fields {
        if key.starts_with("column:") {
          ...
          match column {
            ...
            Either::Right(s) => columns.push(Some((result, s.clone()))),
          }
        }
      }
          ...
          for rule in md.rules {
            if let Either::Left(rule) = rule {
                  ...
                  rule: vec![proto::MatchingRule {
                    ...
                    values: Some(prost_types::Struct {
                      fields: rule
                        .values()
                        .iter()
                        .map(|(key, val)| (key.to_string(), to_value(val)))
                        .collect(),
                    }),
                  }],
                },
              ...
              return Ok(Response::new(proto::ConfigureInteractionResponse {
                error: format!(
                  "Expected a matching rule definition, but got an un-resolved reference {:?}",
                  rule
                ),
...
pub fn generate_csv_content(
  ...
  for (key, gen) in &request.generators {
    ...
    let values = gen
      .values
      .as_ref()
      .ok_or(anyhow!("Generator values were expected"))?
      .fields
      .iter()
    ...
    for (col, field) in record.iter().enumerate() {
      ...
      if has_headers {
        if let Some(generator) =
          generators.get(&Right(headers.get(col).unwrap_or_default().to_string()))
        {
  ...
  Ok(OptionalBody::Present(
    bytes,
    Some(ContentType::from("text/csv;charset=UTF-8")),
    None,
  ))
}
