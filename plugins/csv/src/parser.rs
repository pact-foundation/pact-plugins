  ...
  if first == Some(FieldToken::Column) {
    ...
    if second == Some(FieldToken::Colon) {
      ...
      if let Some(FieldToken::Int(i)) = third {
        if i < 1 {
          Err(anyhow!(
            "'{}' is not a valid field definition, expected an integer >= 1, got {}",
            s,
            i
          ))
        } else {
        ...
        Err(anyhow!(
          "'{}' is not a valid field definition, expected an integer, got '{}'",
          s,
          lex.remainder()
        ))
      }
      ...
      Err(anyhow!(
        "'{}' is not a valid field definition, expected ':', got '{}'",
        s,
        lex.remainder()
      ))
    }
    ...
    Err(anyhow!(
      "'{}' is not a valid field definition, expected 'column', got '{}'",
      s,
      lex.remainder()
    ))
  }
