...
pub fn to_value(value: &Value) -> prost_types::Value {
  match value {
    ...
    Value::Array(v) => prost_types::Value {
    ...
    Value::Object(m) => prost_types::Value {
      kind: Some(prost_types::value::Kind::StructValue(prost_types::Struct {
        fields: m
          .iter()
          .map(|(key, val)| (key.clone(), to_value(val)))
          .collect(),
      })),
    },
  }
...
pub fn from_value(value: &prost_types::Value) -> Value {
  match value.kind.as_ref().unwrap() {
    ...
    Kind::ListValue(l) => Value::Array(l.values.iter().map(|v| from_value(v)).collect()),
  }
}
